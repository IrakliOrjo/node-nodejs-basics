...
export function serveDelete(req: IncomingMessage,res: ServerResponse,userId: string | undefined,users: Record<string, any>) {
     if(cluster.isWorker){
    ...
    if(isValidUUID(userId)){
        ...
        if(user){
            try{
                let index = users.findIndex((item:DatabaseUser) => item.userId === user.userId)
